...
export function ProductPageLayout({
  ...
  const [dbContent, setDbContent] = useState<ProductContentOverride | null>(null)
  ...
  useEffect(() => {
    ...
    const loadDbContent = async () => {
      try {
        ...
        if (mounted && data?.content && typeof data.content === "object") {
          setDbContent(data.content as ProductContentOverride)
          return
        }
  ...
  const effectiveTitle = dbContent?.title ?? title
  const effectiveSubtitle = dbContent?.subtitle ?? subtitle
  const effectiveDescription = dbContent?.description ?? description
  const effectivePlans = Array.isArray(dbContent?.plans) ? dbContent.plans : plans
  const effectiveFaqs = Array.isArray(dbContent?.faqs) ? dbContent.faqs : faqs
  const effectiveBenefits = Array.isArray(dbContent?.benefits) ? dbContent.benefits : benefits
  const effectiveTechSpecs = Array.isArray(dbContent?.techSpecs) ? dbContent.techSpecs : techSpecs
  const effectiveHideQuickBenefits = typeof dbContent?.hideQuickBenefits === "boolean" ? dbContent.hideQuickBenefits : hideQuickBenefits
  const effectiveHideHeroButtons = typeof dbContent?.hideHeroButtons === "boolean" ? dbContent.hideHeroButtons : hideHeroButtons

  const effectiveFeatures = useMemo(() => {
    if (!Array.isArray(dbContent?.features)) {
      return features
    }

    return dbContent.features.map((feature, index) => ({
      icon: features[index]?.icon ?? Zap,
      title: feature.title,
      description: feature.description,
    }))
  }, [dbContent?.features, features])
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <Badge variant="secondary" className="mb-4 bg-primary/10 text-primary border-primary/20">
                  {translateProductText(effectiveSubtitle, language)}
                </Badge>
                <h1 className="text-4xl font-bold tracking-tight text-foreground sm:text-5xl lg:text-6xl">
                  {translateProductText(effectiveTitle, language)}
                </h1>
                <p className="mt-6 text-lg text-muted-foreground max-w-xl">
                  {translateProductText(effectiveDescription, language)}
                </p>
                
                {!effectiveHideQuickBenefits && (
              ...
              <div className="relative hidden lg:flex items-center justify-center">
                ...
                <div className="relative flex h-80 w-80 items-center justify-center rounded-3xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20">
                  {imageSrc ? (
                    <Image
                      src={imageSrc}
                      alt={translateProductText(effectiveTitle, language)}
                      width={160}
                      height={160}
              ...
              {effectivePlans.map((plan, index) => (
                ...
                >
                  ...
                  <CardContent className="flex h-full flex-col">
                    ...
                    <Button
                      className={cn("mt-auto w-full", !plan.popular && "bg-transparent")}
        ...
        {/* Tech Specs Section */}
        {effectiveTechSpecs && effectiveTechSpecs.length > 0 && (
        ...
        {/* Benefits Section */}
        {effectiveBenefits && effectiveBenefits.length > 0 && (
