...
### `site_content` table
Stores editable JSON content per route (plans, sections, prices):

| Column | Type | Description |
|--------|------|-------------|
| `id` | INT (PK) | Auto-increment primary key |
| `path_key` | VARCHAR(255) | Unique route key (e.g. `/hosting/shared`) |
| `content_value` | JSON | Page content override payload |
| `created_at` | TIMESTAMP | Record creation timestamp |
| `updated_at` | TIMESTAMP | Auto-updated on changes |

Use the admin CMS at `/admin/content` to manage this table.
